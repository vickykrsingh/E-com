...
function CreateCategory() {
  ...
  const createCategoryHandler = async (e) => {
    e.preventDefault();
    try {
      const newCategory = await axios.post('/api/v1/category/create-category',{name})
        ...
        <div className='text-white container-fluid'>
            <div className="row pt-5">
              ...
              <Modal  open={isModalOpen} onOk={handleOk} onCancel={handleCancel} footer={null} >
                <CategoryInput createCategoryHandler={handleUpdate}  name={updated}  setName={setUpdated} />
              </Modal>

                <h2 className='text white mb-3'>Create Category</h2>
                <CategoryInput createCategoryHandler={createCategoryHandler}  name={name}  setName={setName}  />
                      ...
                      <tbody className="table-group-divider">
                        { categories.map((c,index)=>(
                          <tr key={c._id} >
                            ...
                            <td>{c.name}</td>
                            <td ><FiEdit onClick={()=>{showModal() ; setUpdated(c.name) ; setSelected(c)}} />  <ImBin style={{marginLeft:'20px'}} onClick={(e)=>{ deleteCategoryHandler(c._id)}} /> </td>
                          </tr>
                        )) }
                      </tbody>
                    </table>
